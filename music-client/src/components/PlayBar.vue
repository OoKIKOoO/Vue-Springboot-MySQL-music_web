        ...
        <div class="kongjian">
            ...
            <div class="playing-speed">
                ...
                <div class="item item-volume">
                    ...
                    <el-slider class="volume" v-model="volume" :show-tooltip="false" :vertical="true"></el-slider>
                </div>
                <!-- 收藏 -->
...
import { download,setCollect,getCollectOfUserId,delCollect,isadd,setPlayCount,addPlayCount} from '../api/index';
    ...
    methods: {
        ...
        togglePlayModel(){
          ...
          if(this.playModel==2){
            ...
            this.playModel==1?this.$store.commit('setPlayModelSVG','#1092'):this.$store.commit('setPlayModelSVG','#1232');
          }
          console.log("playModel: "+this.playModel);
          console.log("playModelSVG: "+this.playModelSVG);
        },
        //上一首
        prev(){
            if(this.listIndex != -1 && this.listPlayList.length > 1){    //当前处于不可能状态或者只有只有一首音乐的时候不执行）
                if(this.playModel==1){  //单曲循环
                ...
                this.toplay(this.listPlayList[this.listIndex].url);
            }
        },
        //下一首
        next(){
            if(this.listIndex != -1 && this.listPlayList.length > 1){    //当前处于不可能状态或者只有只有一首音乐的时候不执行）
                if(this.playModel==1){  //单曲循环
                  ...
                  this.$store.commit('setChangeTime',0);
                  this.$store.commit('setListIndex',this.listIndex);
                }else if(this.playModel==0){                  //列表循环
                  if(this.listIndex < this.listPlayList.length - 1){                                 //不是最后一首音乐
        ...
        toplay: function(url){
            ...
            if(url && url != this.url){
                ...
                this.$store.commit('setPicUrl',this.$store.state.configure.HOST+this.listPlayList[this.listIndex].pic);
                this.$store.commit('setTitle',this.replaceFName(this.listPlayList[this.listIndex].name));
                this.$store.commit('setArtist',this.replaceLName(this.listPlayList[this.listIndex].name));
                this.$store.commit('setLyric',this.parseLyric(this.listPlayList[this.listIndex].lyric));
                this.$store.commit('setIsPlay',true);
