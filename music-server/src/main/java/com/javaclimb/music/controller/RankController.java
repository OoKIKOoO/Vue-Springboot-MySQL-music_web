...
public class RankController {
     ...
     * 根据用户id、歌单id查询评分
     */
    @RequestMapping(value = "/isrank",method = RequestMethod.GET)
    public Object selectUserRank(HttpServletRequest request){
//        if(request.getParameter("consumerId")=="null"||request.getParameter("consumerId")==""){
//            return false;
//        }
        if(request.getParameter("consumerId")!=null&& !"".equals(request.getParameter("consumerId")) &&!"null".equals(request.getParameter("consumerId"))){
            String consumerId = request.getParameter("consumerId");
            String songListId = request.getParameter("songListId");
            Integer score = rankService.selectUserRank(Integer.parseInt(consumerId),Integer.parseInt(songListId));
            if (score==null){
                return false;
            }else{
                return score;
